...
public class MemberRepositoryV4_2 implements MemberRepository {
	...
	public Member save(Member member) {
		...
		try {
			...
			throw exTranslator.translate("save", SAVE_QUERY, e);
		} finally {
	...
	public Member findById(String memberId) {
		...
		try {
			...
			log.error("DB error", e);
			throw exTranslator.translate("findById", search_query, e);
		} finally {
			// 만약 final에서 예외가 터진다면 예외가 밖으로까지 나감 -> try-catchansdmfh cjflgownjdigka
	...
	public void update(String memberId, int money) {
		...
		try {
			...
			log.error("DB error", e);
			throw exTranslator.translate("update", update_query, e);
		} finally {
			// 만약 final에서 예외가 터진다면 예외가 밖으로까지 나감 -> try-catchansdmfh cjflgownjdigka
			close(connection, preparedStatement, null);
		}
	...
	public void delete(String memberId) {
		...
		try {
			...
			log.error("DB error", e);
			throw exTranslator.translate("delete", delete_query, e);
		} finally {
			close(connection, preparedStatement, null);
		}
	}
